...
describe('MoodFilm App Regression Test', () => {
  beforeEach(() => {
    cy.visit('https://deploy-preview-1.moodfilm.pl/');
  });
  ...
  context('Mood Selection', () => {
    it('should display a warning when more than two moods are selected', () => {
      ...
      cy.contains('Możesz wybrać maksymalnie dwa nastroje.').should('be.visible');
    });
  ...
  context('Platform Selection', () => {
    it('should require at least one platform to be selected', () => {
      ...
      // Then a warning should appear
      cy.contains('Wybierz przynajmniej jedną platformę streamingową.').should('be.visible');
    });
  ...
  context('Movie Search', () => {
    ...
    it('should handle API errors gracefully', () => {
      ...
      cy.contains('Wystąpił błąd podczas wyszukiwania filmów. Spróbuj ponownie!').should('be.visible');
    });
    ...
    it('should handle no movies found gracefully', () => {
      ...
      cy.get('.mood-button .btn-outline-light').contains('Nostalgiczny').click();
      ...
      cy.wait('@getNoMovies');
      cy.contains('Nie znaleziono filmów spełniających kryteria.').should('be.visible');
    });
